...
#define GENERATION_SIZE 50 //250번쯤 해야 fitness가 20인 population을 찾아내는 확률이 높아짐
...
printf("randomPopulation1: %d, randomPopulation2: %d \n", rando1, rando2);
...
printf("reIndex: %d, reFitness: %d \n", replacementIndex, replacementFitness);
...
void bitflipMutation(int offspring[ARRAY_SIZE]) {
	...
	for (int i = 0; i < ARRAY_SIZE; i++) {
		...
		if (randomArr[i] < 100) offspring[i] = 1 - offspring[i]; // Mutation Probability를 0.1로 설정, offspring의 값이 0이면 1로, 1이면 0으로 바꿔서 돌연변이 생성
	}
